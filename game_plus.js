/* game_plus.js — Addition game, namespaced as PL for Spielkiste */

var PL = (function() {
  'use strict';

  var STORAGE_ERRORS = 'plus_errors';
  var STORAGE_HISTORY = 'plus_history';
  ...
  // DOM refs
  var addendAEl, addendBEl, answerDisplay, feedback, promptText;
  ...
  function renderHistory() {
    var wrap = document.getElementById('pl-history-list'); if (!wrap) return;
  ...
  // === Build solution path ===
  // For 43 + 12: decompose 12 into ones=2, tens=10 → "43 + 2 = 45, dann 45 + 10 = 55"
  // For 234 + 321: decompose into ones=1, tens=20, hundreds=300
  ...
  function selectCount(btn) {
    ...
    var fgBtn = document.getElementById('pl-fg-btn');
    if (fgBtn) fgBtn.classList.remove('selected');
    var btns = document.querySelectorAll('#pl-count-selector .count-btn');
  ...
  function selectRange(btn) {
    var btns = document.querySelectorAll('#pl-range-selector .count-btn');
    for (var i = 0; i < btns.length; i++) btns[i].classList.remove('selected');
    btn.classList.add('selected');
  ...
  function selectFastGeschafft() {
    ...
    document.getElementById('pl-start-count').textContent = errors.length + ' Aufgaben';
  }

  function updateStartInfo() {
    if (fastGeschafftMode) return;
    document.getElementById('pl-start-count').textContent = selectedCount + ' Aufgaben im ' + selectedRange + 'er Bereich';
  }

  function updateFastGeschafft() {
    var errors = getWeightedErrors();
    var btn = document.getElementById('pl-fg-btn'), info = document.getElementById('pl-fg-info');
    if (!btn) return;
  ...
  // === Generate random addition task ===
  function generateTask(maxVal) {
    // Both a and b >= 1, and a + b <= maxVal
    var minA = Math.max(1, Math.ceil(maxVal * 0.05));
    var maxA = maxVal - 1; // leave room for b >= 1
    var a = minA + Math.floor(Math.random() * (maxA - minA + 1));
    var maxB = maxVal - a;
    if (maxB < 1) maxB = 1;
  ...
  function startGame() {
    ...
    document.getElementById('pl-start-screen').classList.add('hidden');
    document.getElementById('pl-game-screen').classList.remove('hidden');

    addendAEl = document.getElementById('pl-factor-a');
    addendBEl = document.getElementById('pl-factor-b');
    answerDisplay = document.getElementById('pl-answer');
    feedback = document.getElementById('pl-feedback');
    promptText = document.getElementById('pl-prompt');
    scoreText = document.getElementById('pl-score-text');
    progressFill = document.getElementById('pl-progress-fill');
    questionArea = document.getElementById('pl-question-area');
    endScreen = document.getElementById('pl-end');
    endTitle = document.getElementById('pl-end-title');
    endMessage = document.getElementById('pl-end-msg');
    endStats = document.getElementById('pl-end-stats');
    mouseImg = document.getElementById('pl-mouse-img');
    particlesEl = document.getElementById('pl-particles');
    breakScreen = document.getElementById('pl-break');
    breakImage = document.getElementById('pl-break-img');
    breakTimer = document.getElementById('pl-break-timer');
    breakTimerFill = document.getElementById('pl-break-fill');
  ...
  function backToStart() {
    ...
    document.getElementById('pl-scene').classList.remove('hidden');
    ...
    document.getElementById('pl-game-screen').classList.add('hidden');
    document.getElementById('pl-start-screen').classList.remove('hidden');
    initStart();
  ...
  function showBreak() {
    ...
    document.getElementById('pl-scene').classList.add('hidden');
  ...
  function endBreak() {
    ...
    document.getElementById('pl-scene').classList.remove('hidden'); loadQuestion();
  }
  ...
  function loadQuestion() {
    ...
    questionNumber++;
    var t = tasks[queue[0]];
    addendAEl.textContent = t.a; addendBEl.textContent = t.b;
  ...
  function showEnd() {
    ...
    if (firstTryPct === 100) { endTitle.textContent = 'Perfekt!'; endMessage.textContent = 'Kein einziger Fehler — du bist ein Plus-Profi!'; }
  ...
  function toggleHistory() {
    var list = document.getElementById('pl-history-list'), btn = document.getElementById('pl-history-toggle');
